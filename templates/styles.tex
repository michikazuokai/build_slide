...
\definecolor{myblack}{rgb}{0.1, 0.1, 0.1} % 真っ黒すぎない黒を定義

% --- 3. Beamer 各種テンプレートの再定義  ---

% フッター設定（@@leftfooter@@ は Python で置換）
...
% 例：alertblock のタイトルと本文の色をカスタムする
\setbeamercolor{block title alerted}{bg=red!70, fg=white} % タイトル（背景:濃い赤, 文字:白）
\setbeamercolor{block body alerted}{bg=red!10, fg=black}  % 本文（背景:薄い赤, 文字:黒）

% --- itemize / enumerate の余白調整 ---
% 見出し行の直後に itemize を置いたときの空きを小さくする
\makeatletter
...
% --- ノート・教師用メモの制御 ---
\newif\ifteachermode % (もし定義されていなければ)
%@@teachermode@@

\ifteachermode
    % 教師用モード：ノートを表示し、詳細なレイアウトを定義
    ...
    \setbeamertemplate{note page}{%
        ...
        \vspace{1.2ex}
        {\Large\bfseries
        \ifx\notetitletext\@empty \insertframetitle \else \notetitletext \fi
        }\par
        \vspace{-1.2ex}
