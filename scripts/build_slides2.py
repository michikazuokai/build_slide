...
def main() -> None:
    ...
    ap.add_argument("--hidefooter", action="store_true", help="左フッターを完全に出力しない")
    ap.add_argument("--title", default=None, help="スライドのタイトルを任意に指定（未指定時は自動生成）")
    args = ap.parse_args()
...
#    root = Path(__file__).parent  # build_slide/
    ...
    # 既存のデフォルトタイトル生成を、引数がない場合のみ実行するように変更
    if args.title:
        # 引数 --title で指定された文字列をそのまま使用
        stitle = args.title
    else:
    ...
    if args.hidefooter:
        ...
        # 通常時は、従来通りディレクトリ名を表示するLaTeXコードを入れる
        l_footer_content = r"\scriptsize\color{gray!50} \myfootertext"
    ...
    tex_head = (
        ...
        .replace("@@leftfooter@@", l_footer_content) # 新しいプレースホルダを置換
    )
